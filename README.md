...
클라이언트 → POST /api/transfers
         ...
         ├── TransferTransactionResultService.saveTransferTransactionResultBoth()  [거래 내역 2건]
         └── LedgerService.recordForTransfer()           [원장 기록 4건]
         → 200 OK
```
...
7. **거래 내역**: 송신(OUT)과 수신(IN) 2건의 `TransferTransactionResult`를 저장한다. 같은 `transferTransactionGroupId`로 묶인다.
8. **원장 기록**: 복식 부기 원칙에 따라 4건의 `LedgerEntry`를 기록한다 (아래 [7.7 복식 부기](#77-복식-부기-원장-기록) 참고).
...
├── Voucher 1 (송신 거래)
│   ├── LedgerEntry: DEBIT  / DEBT   → 고객 부채(예금) 감소
│   └── LedgerEntry: CREDIT / ASSET  → 은행 자산(현금) 감소
├── Voucher 2 (수신 거래)
│   ├── LedgerEntry: DEBIT  / ASSET  → 은행 자산(현금) 증가
│   └── LedgerEntry: CREDIT / DEBT   → 고객 부채(예금) 증가
```
